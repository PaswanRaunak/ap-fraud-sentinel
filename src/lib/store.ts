...
export type View = 'landing' | 'dashboard' | 'cases' | 'upload' | 'vendors' | 'runs';
...
export function formatCurrency(amount: number | string, currencyCode: string = 'USD'): string {
  ...
  const cleanCode = (currencyCode || 'USD').toUpperCase().trim();
  const symbol = CURRENCY_SYMBOLS[cleanCode] || (cleanCode ? `${cleanCode} ` : '$');
  const locale = cleanCode === 'INR' ? 'en-IN' : cleanCode === 'EUR' ? 'de-DE' : 'en-US';
...
export const useAppStore = create<AppState>()(
  persist(
    ...
    {
      name: 'apsfs-store',
      // Only persist currency preference; do not persist view so landing page always loads first.
      partialize: (s) => ({ currency: s.currency }) as Partial<AppState>,
    },
  ),
);
