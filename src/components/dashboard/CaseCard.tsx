...
export function CaseCard({ c, onClick }: { c: CaseRecord; onClick?: () => void }) {
  ...
  const getRiskTrackLabel = (score: number) => {
    if (score >= 0.7) return { label: 'BEHAVIORAL ANOMALY', bg: 'bg-amber-100 text-amber-900 border-amber-200' };
    if (score >= 0.4) return { label: 'DOMAIN LOOKALIKE', bg: 'bg-[#FFDAD6] text-[#410002] border-[#FFCDD2]' };
    return { label: 'STANDARD CLEAR', bg: 'bg-[#F1F3F5] text-[#44474E] border-[#E2E5E8]' };
  };
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group flex w-full items-center justify-between rounded-2xl border border-[#E2E5E8] bg-[#F7F8FA] px-4 py-3.5 text-left transition-all duration-150 hover:border-[#C4C7C5] hover:bg-white hover:shadow-xs cursor-pointer',
      )}
    >
