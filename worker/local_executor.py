...
def _build_email_index(email_files: list[str]) -> list[tuple[str, dict]]:
    """Pre-parse all email files once into memory."""
    index = []
...
def _pair_emails_for_invoice(invoice_path: str, email_files: list[str], email_index: list[tuple[str, dict]] | None = None) -> str | None:
    ...
    index = email_index if email_index is not None else _build_email_index(email_files)
    for ep, em in index:
...
async def run_batch(run_id: str, batch_path: str | None = None, limit: int | None = None) -> dict:
    ...
    email_index = _build_email_index(email_files)

    for invoice_path in invoice_files:
        case_id_hint = os.path.splitext(os.path.basename(invoice_path))[0]
        email_path = _pair_emails_for_invoice(invoice_path, email_files, email_index=email_index)
